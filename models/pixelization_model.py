...
class PixelizationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(self.netD_B.parameters(),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D_cls = torch.optim.Adam(self.netD_A.parameters(),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
        ...
        # Load parameters
        print('--------Load AliasNet--------')
        load_path = './checkpoints/alias_net.pth'
    ...
    def backward_D_CLS(self, netD, real, fake):
        ...
        loss_D_real = self.criterionGAN(pred_real, True)
        loss_D_CLS = self.criterionCrossEntropy(out_cls, self.label)
        # Fake
    ...
    def backward_G(self,epoch):
        ...
        self.loss_G_CLS = self.criterionCrossEntropy(out_cls, self.label)
        #print(out_cls,out_cls.shape,self.label.shape,self.label)
        self.loss_G_A = self.criterionGAN(out_src, True)
        # GAN loss D_B(G_B(B))
